...
You → your agent → OpenServ x402 → AllowLatch host
                                      ...
                                      └─ Wallet / AgentKit signs only after ALLOW (+ receipt)
```

- **You** never see `SERV_API_KEY` or `CDP_*`.
- **Your agent** discovers the service via `discoverServices()` → name matching `/allowlatch/i` (listed as **AllowLatch Gate**) and pays an x402 fee (**$0.025** per call), or uses `buy_evaluate_pack` for prepaid credits. That payment is how we attribute / bill usage.
- **AllowLatch host** runs with SERV (Policy Copilot) + optional CDP for host-side execute.
...
- Embed into your agent (AgentKit or similar): [EMBED.md](./EMBED.md).

## Why SERV is on the host

So the wow path is complete: messy mandate → conflicts → injection resisted → gate DENY/ALLOW → SERV explain. End users don't configure Reasoning. Host credits are covered by x402 pricing (fixed fee MVP; evaluate pack available).
...
Agent SDK: [EMBED.md](./EMBED.md) (`assertSpend` + allow-receipt · `createGatedAgentKit`).
